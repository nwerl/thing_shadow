...
void mystring_Callback(const char *pJsonString, uint32_t JsonStringDataLen, jsonStruct_t *pContext) {
    IOT_UNUSED(pJsonString);
    IOT_UNUSED(JsonStringDataLen);
    if(pContext != NULL) {
        ESP_LOGI(TAG, "Delta - mySTRING state changed to %s", (char *) (pContext->pData));
    }
}
...
void aws_iot_task(void *param) {
	...
	jsonStruct_t connectFlag;
	connectFlag.cb = NULL;
	connectFlag.pData = &connected_;
	connectFlag.pKey = "connected";
	connectFlag.type = SHADOW_JSON_BOOL;

	char mystr[100] = "";
	jsonStruct_t mystrHandler;
	mystrHandler.cb = mystring_Callback;
	mystrHandler.pData = mystr;
	mystrHandler.pKey = "mystr";
	mystrHandler.type = SHADOW_JSON_STRING;
	...
	IoT_MQTT_Will_Options lwt = iotMqttWillOptionsDefault;
	lwt.pTopicName = "my/things/SHA32/lwp";
	lwt.topicNameLen = (uint16_t) strlen(lwt.pTopicName);
	lwt.pMessage = "{\"state\": {\"reported\": {\"connected\":\"false\"}}}";
	lwt.msgLen = (int16_t) strlen(lwt.pMessage);
	lwt.qos = QOS1;
	...
	rc = aws_iot_shadow_register_delta(&mqttClient, &mystrHandler);
	if (SUCCESS != rc) {
		ESP_LOGE(TAG, "Shadow Register Delta Error");
	}
	...
	if (SUCCESS == rc) {
		...
		rc = aws_iot_shadow_add_reported(JsonDocumentBuffer,
				sizeOfJsonDocumentBuffer, 1, &connectFlag);
		if (SUCCESS == rc) {
			...
			if (SUCCESS == rc) {
				...
				rc = aws_iot_shadow_update(&mqttClient,
						CONFIG_AWS_EXAMPLE_THING_NAME, JsonDocumentBuffer,
